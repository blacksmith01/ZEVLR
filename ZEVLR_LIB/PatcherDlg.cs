            ...
            for (int itask = 0; itask < taskArr.Length; itask++)
            {
                ...
                taskArr[itask] = Task.Run(() =>
                {
                    for (int ifile = taskIdx; ifile < srcFilePaths.Length; ifile += taskArr.Length)
                    {
                        ...
                        if (ProcessExec.Run("java", $"-jar unluac.jar --rawstring --opmap opmap.txt \"{srcFilePath}\" --output \"{dstFilePath}\"", patchDirPath, out var result))
                        {
            ...
            for (int itask = 0; itask < taskArr.Length; itask++)
            {
                ...
                taskArr[itask] = Task.Run(() =>
                {
                    for (int ifile = taskIdx; ifile < srcFilePaths.Length; ifile += taskArr.Length)
                    {
                        ...
                        if (!ProcessExec.Run(Path.Combine(patchDirPath, "luac5.1.exe"), $"-o {normalLuacFilePath} {srcFilePath}", patchDirPath, out var result))
                        {
                            cts.Cancel(); throw new Exception($"[Err] failed compile {srcFilePath} file.");
                        }

                        if (!ProcessExec.Run("java", $"-jar unluac.jar --rawstring --disassemble \"{normalLuacFilePath}\"", patchDirPath, out result))
                        {
                        ...
                        if (!ProcessExec.Run("java", $"-jar unluac.jar --rawstring --assemble \"{disassembledFilePath}\" --output \"{dstFilePath}\"", patchDirPath, out result))
                        {
